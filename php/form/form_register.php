...
<script>
	$(document).ready(function(){
		...
		$("#reg_form").submit(function(event){
			...
			if($('#reg_mail').val().length>50 || $('#reg_mail').val().length<4 
					|| $('#reg_mail').val().indexOf("@")<0 || $('#reg_mail').val().indexOf(".")<0) {
				$('#reg_mail_status').html("Please insert a valid mail between four and fifty chars.");
				valid=false;
			}
			...
			if(valid==true) {
				...
				$.post('php/return/ret_register.php', $(this).serialize(), function(data){
					...
					if(data ==1) { // Success
						$("#reg_form").hide();
						$('#reg_newmsg').html("You are registered now! You will get logged-in automatically. Website refreshing in 3 seconds...");
...
<form id="reg_form" method="post">
    <fieldset>   
           ...
           <legend><b>Sign In:</b></legend>
            ...
            <label for="password2"><b>Reiterate Password:</b></label>
            ...
            <br>
            <input type="submit" value= "Sign In"  class="button round" />
